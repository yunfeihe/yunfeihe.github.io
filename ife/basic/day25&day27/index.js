...
function showReuslt(dtime, selectedTime) {
    ...
    resultEl.innerHTML = `
            现在距离 ${selectedTime.year}年${selectedTime.month}月${selectedTime.day}日 ${leftX(selectedTime.sec,
    2,
    0
)}:${leftX(selectedTime.hour, 2, 0)}:${leftX(selectedTime.hour, 2, 0)} 还有 ${dtime.day} 天 ${dtime.hour} 小时 ${
    dtime.min
} 分 ${dtime.sec} 秒
    `;
}
